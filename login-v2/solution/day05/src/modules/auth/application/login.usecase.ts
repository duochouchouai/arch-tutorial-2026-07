...
export class LoginUseCase {
  ...
  async execute(input: ValidatedLogin): Promise<LoginResult> {
    const { userAccountRepository, passwordHasher, sessionStore, timeProvider, idGenerator } = this.#deps
    const now = timeProvider.now()

    const row = await userAccountRepository.findByUsername(input.username)
    ...
    if (!passwordMatched) {
      ...
      await userAccountRepository.update(user.toRow())
      throw new InvalidCredentialsError()
    }
