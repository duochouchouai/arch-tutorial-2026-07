...
export class RegisterUseCase {
  ...
  async execute(input: ValidatedRegister): Promise<RegisterResult> {
    const { userAccountRepository, passwordHasher, codeStore, timeProvider, idGenerator, userRegisteredPublisher } =
      this.#deps
    ...
    // 2) 查重 —— 读也走公共端口
    if ((await userAccountRepository.findByUsername(input.username)) !== null) {
      throw new UsernameTakenError()
    }
    if ((await userAccountRepository.findByEmail(email)) !== null) {
    ...
    await userAccountRepository.insert(user.toRow())
