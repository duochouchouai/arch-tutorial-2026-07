...
function build() {
  ...
  userAccountRepository.rows.set(
    'u1',
  ...
  return { time, userAccountRepository, sessionStore, useCase }
}

describe('LoginUseCase', () => {
  it('密码正确：会话入库（TTL=配置常数）、登录信息写回、失败计数清零', async () => {
    const { userAccountRepository, sessionStore, useCase } = build()
    ...
    expect(userAccountRepository.rows.get('u1')?.lastLoginAt).toBe(START)
    expect(userAccountRepository.rows.get('u1')?.failedAttempts).toBe(0)
  })

  it('密码错误：抛统一错误 + 失败计数 +1', async () => {
    const { userAccountRepository, useCase } = build()

    await expect(useCase.execute({ username: 'alice', password: 'wrong' })).rejects.toBeInstanceOf(
      InvalidCredentialsError,
    )
    expect(userAccountRepository.rows.get('u1')?.failedAttempts).toBe(1)
  })
  ...
  it(`连续失败 ${MAX_FAILED_ATTEMPTS} 次锁定；锁定期间正确密码也被拒`, async () => {
    ...
    expect(userAccountRepository.rows.get('u1')?.lockedUntil).toBe(START + LOCK_DURATION_MS)
