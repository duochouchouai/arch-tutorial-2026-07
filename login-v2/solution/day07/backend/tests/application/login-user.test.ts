...
import { UnauthorizedError, LockedError } from '../../src/shared/errors';
...
function createMockRepo(): UserRepository {
  return {
    ...
    // 默认：账户未锁定，失败次数 0，lockCount 0
    getLockStatus: vi.fn().mockResolvedValue({ failedAttempts: 0, lockedUntil: null, lockCount: 0 }),
...
describe('LoginUserUseCase', () => {
  ...
  it('should generate a remember token when rememberMe is true', async () => {
    ...
    repo.getLockStatus = vi.fn().mockResolvedValue({ failedAttempts: 0, lockedUntil: null, lockCount: 0 });
