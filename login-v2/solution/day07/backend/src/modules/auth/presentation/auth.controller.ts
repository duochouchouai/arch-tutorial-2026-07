...
export class AuthController {
  ...
  async forgotPassword(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const input = this.#deps.forgotPasswordValidator.validate(req.body)
      const result = await this.#deps.forgotPasswordUseCase.execute(input)
      res.status(200).json(ok(result))
    } catch (err) {
      next(err)
    }
  ...
  async resetPassword(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const input = this.#deps.resetPasswordValidator.validate(req.body)
      const result = await this.#deps.resetPasswordUseCase.execute(input)
      res.status(200).json(ok(result))
    } catch (err) {
      next(err)
    }
