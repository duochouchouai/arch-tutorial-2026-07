...
function build() {
  ...
  return { time, userAccount, codeStore, eventBus, useCase }
}

describe('RegisterUseCase', () => {
  it('验证码正确时：建账号（哈希落库、时间戳来自时钟）→ 消费验证码 → 发注册事件', async () => {
    const { userAccount, codeStore, eventBus, useCase } = build()
    ...
    const row = userAccount.rows.get('id-1')
    ...
    const event = eventBus.published[0] as UserRegisteredEvent
    expect(event.userId).toBe('id-1')
    // Day 07：载荷自带订阅方要的数据（notifications 不必回查 users 模块）
  ...
  it('用户名已占用：抛 UsernameTakenError', async () => {
    const { codeStore, userAccount, useCase } = build()
    await codeStore.save(CODE_PURPOSE_REGISTER, 'alice@example.com', '123456', 2_000)
    userAccount.rows.set('existing', {
      id: 'existing',
      username: 'alice',
