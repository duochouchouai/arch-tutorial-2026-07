...
你已经完成了 day01~day06 的手打学习。今天不再有 Step 1/2/3——只有需求描述。你需要自己判断"改哪一层、改几个文件"。

---

## 🎯 大前提

1. 从 `develop` 创建你自己的分支（命名为 `feat/day07-姓名`）
2. 完成后 PR 到远程 `develop`
3. PR 的 **comparing changes** 必须写清楚：改了哪些文件、哪些层、为什么
...
| 项目 | 内容 | 分数 |
|------|------|------|
| 必做 1：后端需求变更 | 1 个业务需求：递进式锁定 | 50 |
| 必做 2：uniapp 前端 | 对接后端接口，清洁架构分层，UI 不计分 | 50 |
